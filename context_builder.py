...
def _compute_context(raw: dict, user_id: str) -> dict:
    ...
    shop_lookup = {s["$id"]: s.get("name", "Unknown Shop") for s in shops if s.get("$id")}
    ...
    if shop_dues:
        ...
        highest_due_shop = {
            "name": shop_lookup.get(top_sid, "Unknown Shop"),
            "amount": shop_dues[top_sid],
        }

    shop_due_lines = sorted(
        [{"name": shop_lookup.get(sid, f"Shop {sid[:6]}"), "due": due}
    ...
    recent_transactions = [
        {
            "shop": shop_lookup.get(t.get("business_id", ""), "Unknown Shop"),
            "amount": float(t.get("amount", 0)),
...
def get_user_context(
    ...
    if cache_key in _context_cache:
        ...
        if time.time() - cached_ts < CACHE_TTL_SECONDS:
            ...
            if query:
                from rag_service import search_user_data
                ctx["personal_notes"] = search_user_data(query=query, user_id=user_id, top_k=3)
            else:
                ctx["personal_notes"] = []
    ...
    if query:
        from rag_service import search_user_data
        ctx["personal_notes"] = search_user_data(query=query, user_id=user_id, top_k=3)
        logger.info(f"🔎 Found {len(ctx['personal_notes'])} personal semantic matches for query.")
    else:
        ctx["personal_notes"] = []
